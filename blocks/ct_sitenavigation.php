...
function site_block_nav($options)
{
    ...
    $result = $xoopsDB->query("SELECT title, storyid, parent_id, homepage, nohtml, nosmiley, nobreaks, nocomments, link, address, submenu, visible, blockid AS priority, 'content' AS type FROM " . $xoopsDB->prefix('content') . ' where visible = 1 ORDER BY blockid');
    ...
    if ($xoopsModule && ('Content' == $xoopsModule->name() || 'content' == $xoopsModule->dirname())) {
        ...
        $result = $xoopsDB->query('SELECT storyid FROM ' . $xoopsDB->prefix('content') . ' WHERE visible=1 AND assoc_module = ' . $xoopsModule->getVar('mid'));

        if ($xoopsDB->getRowsNum($result) > 0) {
            [$currentpage] = $xoopsDB->fetchRow($result);
        }
    }
...
function print_menu($menuItems, &$fullList, $level, &$depth)
{
    ...
    foreach ($menuItems as $menuItem) {
        ...
        if ('content' == $menuItem['type']) {
            ...
            if ($children) {
                if (in_array($menuItem['storyid'], $allParents, true)) {
    ...
    if (0 == $level) {
        if (is_object($GLOBALS['xoopsUser']) && $GLOBALS['xoopsUser']->isAdmin()) {
            $MyList .= "\n\t<a class=\"menuMain\" href=\"" . XOOPS_URL . '/modules/content/admin/index.php?op=submit&id=0&return=1' . '">' . _C_MENUADDITEM . '</a>';
        }
    }
...
{
    $form = '&nbsp;' . _C_B_PADDING . '&nbsp;<input type="text" name="options[]" value="' . $options[0] . '" size="5">&nbsp;pixels';

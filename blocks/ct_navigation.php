...
function content_block_nav()
{
    ...
    $result = $xoopsDB->query(
        "SELECT child.link, child.storyid, child.blockid, child.title, child.visible, child.parent_id, child.address,CASE parent.parent_id WHEN 0 THEN parent.blockid ELSE child.blockid END 'menu_block', CASE child.parent_id WHEN 0 THEN child.storyid ELSE child.parent_id END 'menu_id' child.newwindow FROM "
    ...
    if ($xoopsModule) {
        ...
        [$module_id] = $xoopsDB->fetchArray($result);
    }
    ...
    if ($gpermHandler->checkRight('module_admin', $module_id, $groups, 1)) {
        $block['links'][] = ['title' => '<font color="#FF9933">Add main menu item</font>', 'address' => XOOPS_URL . '/modules/content/admin/index.php?op=submit&id=0&return=1', 'parent' => 0, 'currentParent' => $currentParent, 'currentPage' => $_GET['id']];
    }

...
//		sHRef = sHRef.remove( 0, FCK.BaseUrl.length ) ;
	...
	//Update Local Field
	for (i = 0; i < GetE('cmbLocalFileList').options.length; i++){
		elLoc = GetE('cmbLocalFileList').options[i]
		if (elLoc.value == GetE('txtUrl').value){
			GetE('cmbLocalFileList').value = GetE('txtUrl').value
			break;
		}
	}
}
...
function SetURLtoLocal(){
	GetE('txtUrl').value = GetE('cmbLocalFileList').value;
}

...
if (!isset($_FILES['NewFile']) || null === $_FILES['NewFile']['tmp_name'] || '' == $_FILES['NewFile']['name']) {
    SendResults('202');
}
...
if (!in_array($sType, ['File', 'Image', 'Flash', 'Media'], true)) {
...
if ((count($arAllowed) > 0 && !in_array($sExtension, $arAllowed, true)) || (count($arDenied) > 0 && in_array($sExtension, $arDenied, true))) {
    SendResults('202');
}
...
if (isset($Config['UserFilesAbsolutePath']) && mb_strlen($Config['UserFilesAbsolutePath']) > 0) {

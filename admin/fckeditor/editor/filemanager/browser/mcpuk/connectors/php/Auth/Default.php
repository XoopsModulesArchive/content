...
class Auth
{
    public function authenticate($data, $fckphp_config)
    {
        ...
        for ($i = 0; $i < mb_strlen($data) - 1; $i += 2) {
            $decData .= chr(hexdec($data[$i] . $data[$i + 1]));
        }
            ...
            if ($decArray[3] == md5($decArray[0] . '|^SEP^|' . $decArray[1] . '|^SEP^|' . $decArray[2] . $key)) {
                if (time() - $decArray[0] < 3600) { //Token valid for max of 1 hour
                    if ($_SERVER['REMOTE_ADDR'] == $decArray[1]) {
                        ...
                        $fckphp_config['UserFilesPath'] .= '/users/' . $decArray[2];

                        $up = str_replace('//', '/', $fckphp_config['basedir'] . '/' . $fckphp_config['UserFilesPath']);

                        if (!file_exists($top)) {
                            mkdir($top, 0777) || die('users folder in UserFilesPath does not exist and could not be created.');
                        ...
                        if (!file_exists($up)) {
                            mkdir($up, 0777) || die('users/' . $decArray[2] . ' folder in UserFilesPath does not exist and could not be created.');
                        ...
                        foreach ($fckphp_config['ResourceTypes'] as $value) {
                            if (!file_exists("$up/$value")) {
                                mkdir("$up/$value", 0777) || die('users/' . $decArray[2] . "/$value folder in UserFilesPath does not exist and could not be created.");

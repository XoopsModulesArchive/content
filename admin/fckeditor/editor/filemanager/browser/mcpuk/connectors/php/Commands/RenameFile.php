...
class RenameFile
{
    ...
    public function __construct($fckphp_config, $type, $cwd)
    {
        ...
        $this->newname = str_replace(['..', '/'], '', $this->checkName($_GET['NewName']));
    }
    ...
    public function checkName($name)
    {
        ...
        for ($i = 0, $iMax = mb_strlen($name); $i < $iMax; $i++) {
            if (in_array($name[$i], $this->fckphp_config['FileNameAllowedChars'], true)) {
                $newName .= $name[$i];
            }
        }
    ...
    public function nameValid($fname)
    {
        $type_config = $this->fckphp_config['ResourceAreas'][$this->type];

        $lastdot = mb_strrpos($fname, '.');

        if (false !== $lastdot) {
            $ext = mb_substr($fname, ($lastdot + 1));

            $fname = mb_substr($fname, 0, $lastdot);

            if (in_array(mb_strtolower($ext), $type_config['AllowedExtensions'], true)) {
                return true;
            }

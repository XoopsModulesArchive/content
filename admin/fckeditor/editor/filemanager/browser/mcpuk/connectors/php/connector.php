...
function errorHandler($errno, $errstr, $errfile, $errline, $errcontext)
{
    ...
    if (false === mb_strpos($errstr, 'var: Deprecated.')) {
        global $fckphp_config;

        if (true === $fckphp_config['Debug'] && true === $fckphp_config['Debug_Errors']) {
            $oldData = implode('', file($fckphp_config['DebugOutput']));

            if ($fh = fopen($fckphp_config['DebugOutput'], 'wb')) {
                fwrite($fh, "\n" . date('d/m/Y H:i:s') . "\n");
                ...
                if ($fckphp_config['Debug_Trace']) {
                    fwrite($fh, '		Error Context: ' . print_r($errcontext, true) . "\n");
                }

                if ($fckphp_config['Debug_GET']) {
                    fwrite($fh, "\n\$_GET::\n" . print_r($_GET, true) . "\n");
                }

                if ($fckphp_config['Debug_POST']) {
                    fwrite($fh, "\n\$_POST::\n" . print_r($_POST, true) . "\n");
                }

                if ($fckphp_config['Debug_SERVER']) {
                    fwrite($fh, "\n\$_SERVER::\n" . print_r($_SERVER, true) . "\n");
                }

                if ($fckphp_config['Debug_SESSIONS']) {
        ...
        if (!$reported) {
            ...
            if ($fckphp_config['Debug_Trace']) {
                echo 'Error Context: ' . print_r($errcontext, true) . "\n";
            }
...
if (true === $fckphp_config['Debug'] && $fckphp_config['Debug_Output']) {
    ob_start();
}
outputHeaders();
...
$type = (((isset($_GET['Type'])) && ('' != $_GET['Type'])) ? $_GET['Type'] : 'File');

$cwd = str_replace(
    '..',
    '',
    (((isset($_GET['CurrentFolder'])) && ('' != $_GET['CurrentFolder'])) ? $_GET['CurrentFolder'] : '/')
);
...
if (in_array($command, $valid_commands, true)) {
    if ($fckphp_config['auth']['Req']) {
        ...
        if (true !== $fckphp_config['authSuccess']) {
    ...
    if (!in_array($type, $valid_resource_types, true)) {

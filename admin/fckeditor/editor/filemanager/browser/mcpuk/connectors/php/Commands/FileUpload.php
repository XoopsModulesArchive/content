...
class FileUpload
{
    ...
    public function cleanFilename($filename)
    {
        ...
        for ($i = 0, $iMax = mb_strlen($filename); $i < $iMax; $i++) {
            if (in_array(mb_substr($filename, $i, 1), $this->fckphp_config['FileNameAllowedChars'], true)) {
                $n_filename .= mb_substr($filename, $i, 1);
            }
        }
    ...
    public function run()
    {
        ...
        if ((0 == count($_FILES))
            && isset($_GET['file']) && isset($_GET['file']['NewFile'])
            ...
            if (array_key_exists('NewFile', $_FILES)) {
                if ($_FILES['NewFile']['size'] < ($typeconfig['MaxSize'] * 1024)) {
                    ...
                    if (false !== $lastdot) {
                        ...
                        if (in_array(mb_strtolower($ext), $typeconfig['AllowedExtensions'], true)) {
                            $test = 0;
                            ...
                            if ((-1 != $typeconfig['DiskQuota']) && (!$failSizeCheck)) {
                                if (-1 == $this->fckphp_config['DiskQuota']['Global']) {
                                    $dirSizes[$this->type] = $this->getDirSize(
                            ...
                            if (((-1 != $this->fckphp_config['DiskQuota']['Global']) || (-1 != $typeconfig['DiskQuota'])) && $failSizeCheck) {
                                //Disk Quota over
                                ...
                                if (file_exists($this->real_cwd . "/$filename.$ext")) {
                                    ...
                                    for ($i = 1; (($i < 200) && (false === $taskDone)); $i++) {

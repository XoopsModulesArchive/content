...
class CreateFolder
{
    ...
    public function __construct($fckphp_config, $type, $cwd)
    {
        ...
        $this->newfolder = str_replace(['..', '/'], '', $_GET['NewFolderName']);
    }
    ...
    public function checkFolderName($folderName)
    {
        ...
        if (mb_strlen($folderName) > $this->fckphp_config['MaxDirNameLength']) {
            return false;
        }

        //Check that it only contains valid characters

        for ($i = 0, $iMax = mb_strlen($folderName); $i < $iMax; $i++) {
            if (!in_array(mb_substr($folderName, $i, 1), $this->fckphp_config['DirNameAllowedChars'], true)) {
                return false;
            }
        }

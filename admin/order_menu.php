...
if (isset($_GET['op']) && 'order' == $_GET['op']) {
    for ($j = 1; $j <= $_POST['total']; $j++) {
        if ('module' == $_POST['type' . $j]) {
            if (!$result = $xoopsDB->query('UPDATE ' . $xoopsDB->prefix('modules') . " SET weight = '" . $_POST['priority' . $j] . "' WHERE mid = '" . $_POST['id' . $j] . "'")) {
                echo _C_ERRORINSERT;
            }

            //echo "UPDATE ".$xoopsDB->prefix('modules')." SET weight = '".$_POST['priority'.$j]."' WHERE mid = '".$_POST['id'.$j]."'<br>\n";
        } elseif ('content' == $_POST['type' . $j]) {
            if (!$result = $xoopsDB->query('UPDATE ' . $xoopsDB->prefix('content') . " SET blockid = '" . $_POST['priority' . $j] . "' WHERE storyid = '" . $_POST['id' . $j] . "'")) {
                echo _C_ERRORINSERT;
            }

            //echo "UPDATE ".$xoopsDB->prefix('content')." SET blockid = '".$_POST['priority'.$j]."' WHERE storyid = '".$_POST['id'.$j]."'<br>\n";
        }
    }

    redirect_header('order_menu.php', 2, _C_DBUPDATED);
} else {
    print_header();
        ...
        if (in_array($i, $read_allowed, true)) {
            ...
            if ((count($sublinks) > 0) && (!empty($xoopsModule)) && ($i == $xoopsModule->getVar('mid'))) {
                foreach ($sublinks as $sublink) {
                    $menuModule[$i]['sublinks'][] = ['text' => $sublink['name'], 'url' => XOOPS_URL . '/modules/' . $modules[$i]->getVar('dirname') . '/' . $sublink['url']];
                }
            } else {
    ...
    if ($xoopsModule && ('Content' == $xoopsModule->name() || 'content' == $xoopsModule->dirname()) && isset($_GET['id'])) {
        $result = $xoopsDB->query(
            "SELECT CASE parent_id WHEN 0 THEN storyid ELSE parent_id END 'sortorder' FROM " . $xoopsDB->prefix('content') . " WHERE visible='1' AND storyid=" . $_GET['id']
        );

        [$currentParent] = $xoopsDB->fetchRow($result);
    }

    $result = $xoopsDB->query("SELECT storyid, blockid, title, visible, parent_id, address, blockid AS 'menu_block', parent_id AS 'menu_id' FROM " . $xoopsDB->prefix('content') . ' WHERE visible=1 AND parent_id = 0 ORDER BY menu_block, menu_id, parent_id, blockid');
    ...
    while (false !== ($tcontent = $xoopsDB->fetchArray($result))) {
        if (0 == $tcontent['parent_id']) {
            $menu[] = ['text' => htmlspecialchars($tcontent['title'], ENT_QUOTES | ENT_HTML5), 'url' => XOOPS_URL . '/modules/content/index.php?id=' . $tcontent['storyid'], 'priority' => $tcontent['menu_block'], 'id' => $tcontent['storyid'], 'type' => 'content'];

            $j = count($menu);
        } else {//if ($tcontent['parent_id'] == $currentParent){
            $menu[$j - 1]['sublinks'][] = ['text' => htmlspecialchars($tcontent['title'], ENT_QUOTES | ENT_HTML5), 'url' => XOOPS_URL . '/modules/content/index.php?id=' . $tcontent['storyid']];
        }
    }
    ...
    echo '<h4>' . _C_ADMINTITLE . '</h4>' . showMenu() . "
			 <table width='100%' border='0' cellpadding='0' cellspacing='1' class='outer'>
			 	<tr class='even'>
					<td><strong>" . _C_LINKNAME . '</strong></td>
					<td><strong>' . _C_LINKID . "</strong></td>
